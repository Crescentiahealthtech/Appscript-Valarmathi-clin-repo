...
| **Meta, if the WhatsApp Business API is switched on for reminders** | Name, mobile, and the date and kind of visit (appointment, follow-up, child's vaccination). Only when `REMINDER_PROVIDER = WHATSAPP_CLOUD` (`Patient_Reminders.gs`); otherwise reminders go by the desk's own WhatsApp and by Gmail, both covered by the rows above. | Meta's WhatsApp Business terms include a data processing addendum. | ⬜ Only patients with `COMMUNICATION` consent are messaged, automatically or from the desk. |
